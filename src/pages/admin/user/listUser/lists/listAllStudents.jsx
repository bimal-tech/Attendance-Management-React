...
const ListAllStudent = () => {
    ...
    const columns = [
        ...
        {
            name: 'Action',
            selector: row => <ActionButtons dataId={row.id} onDelete={deleteStudent} onEdit={`edit/student/${row.id}`} />,
        },
    ];
    const deleteStudent = async (data) => {
        try {
            let success = await deleteStudentByID(data);
    ...
    const deleteStudentByID = async (id) => {
        try{
            let response = await httpDelete('/delete/student/'+id);
            return response;
        } catch(error) {
            throw error;
        }
